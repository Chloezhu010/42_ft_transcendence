...
See [Architecture](documentation/architecture.md) for full system topology and sequence diagrams.

---

## Database Schema

SQLite, with foreign keys and `ON DELETE CASCADE` on user-owned data. All story/profile/friendship rows are user-scoped.
...
kid_profiles                       stories                              panels
├── id PK                          ├── id PK                            ├── id PK
├── user_id → users(id)            ├── user_id        → users(id)       ├── story_id → stories(id)
├── name                           ├── kid_profile_id → kid_profiles(id)├── panel_order
├── gender (boy/girl/neutral)      ├── title                            ├── text
├── skin_tone, hair_color,         ├── foreword                         ├── image_prompt
│   eye_color, favorite_color      ├── character_description            ├── image_path
├── dream                          ├── cover_image_prompt               └── created_at
├── archetype                      ├── cover_image_path                    UNIQUE(story_id, panel_order)
├── art_style                      ├── created_at
└── created_at                     └── updated_at

api_keys                           oauth_accounts
├── id PK                          ├── id PK
├── user_id → users(id)            ├── user_id  → users(id)
├── key_hash (sha256)              ├── provider (google)
...
- One `user` → many `kid_profiles`, `stories`, `api_keys`, `oauth_accounts`.
- One `kid_profile` → many `stories`.
- One `story` → many `panels` (ordered by `panel_order`, unique within a story).
- `friendships` is a self-join on `users` with a status enum.

Full DDL and rationale: [Architecture → Database Schema](documentation/architecture.md#database-schema).
