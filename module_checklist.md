...
| 3 | Web | Complete notification system | Minor | 1 | ✅ Done | Low | `sonner` v2 wired across auth, profile, friends, gallery, story flows (34+ toast calls across 8 files) |
...
| 5b | Accessibility | Right-to-left (RTL) language support | Minor | 1 | ✅ Done | Low | Arabic fully translated (859 lines); 49 logical-property class usages (`ms-/me-/ps-/pe-/start-/end-`); ratcheting static audit `tests/i18n/rtlAudit.test.ts` bans physical-side classes across `app/`, `components/`, `pages/`; `getDirectionalArrow()` flips arrows; `rtl:` Tailwind variants flip gradients (e.g. `HeroSection.tsx`); `StoryboardView`/`PreviewView` use direction-aware helpers; `i18n.on('languageChanged')` → updates `<html dir>` + `<html lang>` for seamless switching |
