...
function ApiKeyRow({ apiKey, isRevoking, onRevoke }: ApiKeyRowProps): JSX.Element {
  ...
  const createdAt = formatDate(apiKey.created_at);
  const lastUsedAt = formatDate(apiKey.last_used_at);
      ...
      <div className="grid gap-4 lg:grid-cols-[1.3fr_1fr_auto] lg:items-center">
        ...
        <dl className="grid gap-2 text-sm text-brand-muted sm:grid-cols-2 lg:grid-cols-1">
          ...
          <div>
            ...
            <dd>{lastUsedAt ?? t('apiKeys.list.neverUsed')}</dd>
          </div>
        </dl>
...
export function ApiKeysPage(): JSX.Element {
  ...
  async function handleCopyCreatedKey(): Promise<void> {
    ...
    try {
      ...
      toast.error(t('apiKeys.created.copyFailed'));
    }
  }

  function handleRevoke(keyId: number): void {
    if (!window.confirm(t('apiKeys.actions.confirmRevoke'))) return;
    ...
    return (
      <div className="flex flex-1 items-center justify-center py-12">
        <div className="w-full max-w-2xl rounded-2xl border-4 border-red-200 bg-white p-8 shadow-soft">
          ...
          <p role="alert" className="mt-4 text-center text-base font-medium text-red-600">
            {loadError}
          </p>
        </div>
      </div>
    ...
    <div className="flex flex-1 justify-center py-12">
      <div className="w-full max-w-5xl space-y-8 rounded-2xl border-4 border-brand-primary/20 bg-white p-6 shadow-soft md:p-10">
        <div className="space-y-2 text-center">
          <p className="text-sm font-bold uppercase tracking-[0.28em] text-brand-primary">
            {t('apiKeys.header.eyebrow')}
          </p>
        ...
        >
          ...
          <div className="mt-3 grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
            ...
            <SketchyButton
              type="submit"
              className="px-6 py-3 text-base disabled:cursor-not-allowed disabled:opacity-50"
              disabled={isCreating}
            >
          ...
          <section className="rounded-2xl border-4 border-brand-accent bg-brand-accent/10 p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div className="min-w-0 flex-1">
                ...
                <code
                  dir="ltr"
                  className="mt-4 block max-h-32 overflow-auto break-all rounded-xl bg-white p-4 font-mono text-sm text-brand-dark"
                >
                  {createdKey}
                </code>
              </div>
